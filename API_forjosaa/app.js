...
app.get('/api/predicted-cutoffs', async (req, res) => {
  try {
    const limit = parseInt(req.query.limit) || 100;
    const page = parseInt(req.query.page) || 1;
    const start = (page - 1) * limit;
    ...
    const predictedData = await dataLoader.loadPredictedData();
    res.json({
      total: predictedData.length,
      page,
      limit,
      data: predictedData.slice(start, end)
    });
  } catch (error) {
    res.status(500).json({ error: 'Failed to load predicted data' });
  }
});
